...
export default function TournamentView() {
  ...
  const [participants, setParticipants] = useState<Participant[]>([]) // Need endpoint for this
  const [matches, setMatches] = useState<Match[]>([]) // Need endpoint for this
  const { user } = useAuth()
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {tournament.status === 'active' && isAdmin && tournament.currentRound >= tournament.totalRounds && (
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {canReport && !match.result && !match.isBye && (
                          <Button size="sm" variant="outline" onClick={() => {
                            const score1Input = prompt(`Score for ${p1?.username || p1?.guestName || 'Player 1'}`)
                            const score2Input = prompt(`Score for ${p2?.username || p2?.guestName || 'Player 2'}`)
                            
                            if (score1Input === null || score2Input === null) return
                        ...
                        {isAdmin && match.result && isCurrentRound && tournament.status !== 'completed' && (
                          <Button size="sm" variant="ghost" className="text-zinc-500 hover:text-white" onClick={() => {
                            const score1 = prompt(`Edit Score for ${p1?.username || p1?.guestName || 'Player 1'}`, match.result?.split('-')[0])
                            const score2 = prompt(`Edit Score for ${p2?.username || p2?.guestName || 'Player 2'}`, match.result?.split('-')[1])
                            ...
                            api(`/matches/${match.id}`, {
                              method: 'PUT',
                              body: JSON.stringify({
                                winnerId: s1 > s2 ? match.player1Id : (s2 > s1 ? match.player2Id : null),
                                result: `${s1}-${s2}`,
                                createdBy: user?.id
                              })
                            }).then(loadTournament).catch(e => alert(e.message))
                          }}>
                            Edit
                          </Button>
                        )}
